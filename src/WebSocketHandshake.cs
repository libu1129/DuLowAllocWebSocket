...
public sealed class WebSocketHandshake
{
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            int port = uri.IsDefaultPort ? (uri.Scheme == "wss" ? 443 : 80) : uri.Port;
            await socket.ConnectAsync(new IPEndPoint(addresses[0], port), ct).ConfigureAwait(false);
            ...
            string request =
                ...
                $"Sec-WebSocket-Extensions: {CompressionNegotiator.BuildClientOfferHeader()}\r\n" +
                "\r\n";
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    var (accepted, compression) = ValidateResponse(headerText, secKey);
                    if (!accepted)
                    {
    ...
    private static (bool Accepted, CompressionOptions Compression) ValidateResponse(string responseHeaders, string secKey)
    {
        ...
        CompressionOptions compression = extensions is null
            ? new CompressionOptions(false, false, false, null)
